...
from ramses_tx.protocol import RamsesProtocol, protocol_factory
...
@pytest.fixture()
async def protocol(rf: VirtualRf) -> AsyncGenerator[RamsesProtocol, None]:
    ...
    assert isinstance(protocol, RamsesProtocol)  # mypy
    ...
    transport = await transport_factory(
        protocol, port_name=rf.ports[0], port_config={}, extra={"virtual_rf": rf}
    )  # injected to aid any debugging
...
def assert_protocol_state_detail(
    protocol: RamsesProtocol, cmd: Command | None, num_sends: int
) -> None:
    assert isinstance(protocol._context, ProtocolContext)  # mypy
...
async def _test_flow_30x(protocol: RamsesProtocol) -> None:
    assert (
    ...
    task = rf._loop.create_task(protocol.send_cmd(II_CMD_0, qos=qos), name="send_1")
    assert await task == II_CMD_0  # no reply pkt expected

    # STEP 2: Send an RQ cmd, then receive the corresponding RP pkt...
    task = rf._loop.create_task(protocol.send_cmd(RQ_CMD_0, qos=qos), name="send_2")
    ...
    task = rf._loop.create_task(protocol.send_cmd(II_CMD_0, qos=qos), name="send_3A")
    assert await task == II_CMD_0  # no reply pkt expected

    task = rf._loop.create_task(protocol.send_cmd(II_CMD_0, qos=qos), name="send_3B")
    assert await task == II_CMD_0  # no reply pkt expected

    # STEP 4: Send an RQ cmd, then receive the corresponding RP pkt...
    task = rf._loop.create_task(protocol.send_cmd(RQ_CMD_1, qos=qos), name="send_4A")
    # sk = rf._loop.create_task(protocol.send_cmd(RQ_CMD_1, qos=qos), name="send_4B")
...
async def _test_flow_401(protocol: RamsesProtocol) -> None:
...
async def _test_flow_402(protocol: RamsesProtocol) -> None:
    qos = QosParams(wait_for_reply=False)
...
async def _test_flow_qos_helper(
    ...
    try:
        ...
        assert False, f"Had expected {ProtocolSendFailed}"


async def _test_flow_60x(protocol: RamsesProtocol, num_cmds: int = 1) -> None:
    #
    # Setup...
    tasks = []
    for idx in range(num_cmds):
        cmd = Command.get_zone_temp("01:123456", f"{idx:02X}")
        coro = protocol.send_cmd(cmd, qos=QosParams(wait_for_reply=False))
        tasks.append(protocol._loop.create_task(coro, name=f"cmd_{idx:02X}"))

    assert await asyncio.gather(*tasks)


async def _test_flow_qos(protocol: RamsesProtocol) -> None:
    assert isinstance(protocol._context, ProtocolContext)  # mypy

    # HACK: to reduce test time
    protocol._context.SEND_TIMEOUT_LIMIT = 0.01  # type: ignore[misc]
    ...
    pkt = await protocol.send_cmd(cmd, qos=QosParams(wait_for_reply=None))
    assert pkt == cmd, "should be echo as there is no wait_for_reply"

    cmd = Command.put_sensor_temp("03:000555", 19.5)
    pkt = await protocol.send_cmd(cmd, qos=QosParams(wait_for_reply=False))
    assert pkt == cmd, "should be echo as there is no wait_for_reply"

    cmd = Command.put_sensor_temp("03:000666", 19.5)
    pkt = await protocol.send_cmd(cmd, qos=QosParams(wait_for_reply=True))
    assert pkt == cmd, "Should be echo as there's no reply to wait for"

    # # ### Simple test for an RQ (expects an RP)...

    cmd = Command.get_system_time("01:000111")
    pkt = await protocol.send_cmd(cmd)
    assert pkt == cmd, "Should be echo as there's no reply to wait for"

    cmd = Command.get_system_time("01:000222")
    pkt = await protocol.send_cmd(cmd, qos=None)
    assert pkt == cmd, "Should be echo as there's no reply to wait for"
    ...
    coro = protocol.send_cmd(cmd, qos=QosParams(wait_for_reply=True, timeout=0.05))
